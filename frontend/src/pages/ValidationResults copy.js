  ...
  invalidRows.forEach(row => {
    ...
    errors.forEach(err => {
      if (
        ...
        // Normalize message: strip trailing `. Got: "..."` so it reads cleanly
        // when multiple values are grouped under the same rule card
        const message = rawMessage.replace(/\.\s*Got:\s*"[^"]*"\.?$/, '');

        // Group by rule only — all values for the same rule go into one card
        const key = rule;
                        ...
                        {group.values.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {group.values.map((v, i) => (
                              <span key={i} className="px-2.5 py-0.5 rounded-md text-xs font-mono bg-amber-50 text-amber-800 border border-amber-200">
                                {v}
                              </span>
                            ))}
                          </div>
      ...
      cell: (row) => (
        <div className="flex items-center justify-end gap-2 flex-nowrap">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white bg-green-500 hover:bg-green-600 transition-colors whitespace-nowrap"
          >
          ...
          {row.totalFail > 0 && (
            <>
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-rose-100 text-rose-700 hover:bg-rose-200 border border-rose-200 transition-colors whitespace-nowrap"
              >
                <XCircleIcon size={14} />
                View Errors
              </button>
